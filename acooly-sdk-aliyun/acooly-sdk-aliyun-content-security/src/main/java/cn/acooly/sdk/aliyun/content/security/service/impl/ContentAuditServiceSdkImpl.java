...
import cn.acooly.sdk.aliyun.content.security.domain.TextAuditResponse;
import cn.acooly.sdk.aliyun.content.security.domain.TextAuditResult;
import cn.acooly.sdk.aliyun.content.security.enums.AduitLabel;
import cn.acooly.sdk.aliyun.content.security.enums.AliyunComError;
import cn.acooly.sdk.aliyun.content.security.enums.ContentAuditError;
import cn.acooly.sdk.aliyun.content.security.enums.Suggestions;
...
public class ContentAuditServiceSdkImpl implements ContentAuditService {
	...
	public TextAuditExtResponse textScanExt(TextAuditExtRequest request) {
		TextAuditExtResponse textAuditExtResponse = new TextAuditExtResponse();
		...
		String requestJson = JSON.toJSONString(request);
		log.debug("文字审计ext 请求: {}", requestJson);
		textScanRequest.setHttpContent(requestJson.getBytes(), "UTF-8", FormatType.JSON);
		// 请务必设置超时时间。
		textScanRequest.setConnectTimeout(3000);
		textScanRequest.setReadTimeout(6000);
		try {
			...
			List<TextAuditResponse> forbiddenTexts = Lists.newArrayList();
			for (TextAuditResponse textAuditResponse : responses) {
				if (Strings.isNotBlank(textAuditResponse.getFilteredContent())) {
					textAuditExtResponse.setAuditResult(ContentAuditError.AUDIT_BLOCK);
					forbiddenTexts.add(textAuditResponse);
				}
			}
			textAuditExtResponse.setForbiddenContents(forbiddenTexts);
		} catch (ServerException e) {
			throw new BusinessException(e.getErrCode(), e.getErrMsg(), e);
		} catch (ClientException e) {
			throw new BusinessException(e.getErrCode(), e.getErrMsg(), e);
	...
	private void handleResult(JSONObject jsonObject) {
		...
		if (!AliyunComError.OK.getCode().equals(code)) {
			...
			throw new BusinessException(aliyunComError != null ? aliyunComError.name() : String.valueOf(code),
					responseMsg, "");
		}
	}
